...
class LikeController {
  ...
  Future<String> likeCheckListSave(LikeSaveDeleteSendDto likeSaveSendDto) async {
    ServerConnector serverConnector = ServerConnector();

    try {
      await serverConnector.sendProcess("/like/create", json.encode(likeSaveSendDto.toJson()));
  ...
  Future<String> likeCheckListDelete(LikeSaveDeleteSendDto likeSaveSendDto) async {
    ServerConnector serverConnector = ServerConnector();

    try {
      await serverConnector.sendProcess("/like/delete", json.encode(likeSaveSendDto.toJson()));
