...
public class HomeService {
	...
	public HomeDto homeInformationLoadService(String userid) {
		...
		List<HealthRecord> userHealthRecord = healthRecordRepository.findByMemberUseridAndHealthdateBetween(
				userid,
		...
		if(userHealthRecord.size() == 1) {
			...
			homeDto.setConsumeCalories(
					cameraRepository.findByMemberUseridAndCameradateBetween(
							userid,
							LocalDate.now().toString(),
							LocalDate.now().plusDays(1).toString()).get(0).getCalories()
			);
		} else {
	...
	private HealthRecord userTodayNewHealthRecordCreate(String userid) {
		...
		newHealthRecord.setHealthdate(LocalDate.now().toString()); // 날짜 자동 업데이트
		newHealthRecord.setWaterintake(0); // 0
		newHealthRecord.setSteps(0); // 0
